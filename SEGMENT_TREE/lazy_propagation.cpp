...
public:
    ...
    void update(int n, int l, int r, int rl, int rr, int val)
    {
        if (l >= rl && r <= rr)
        {
            apply(n, l, r, val);
        ...
        int m = (l + r) / 2;
        update(2 * n, l, m, rl, rr, val);
        update(2 * n + 1, m + 1, r, rl, rr, val);
        t[n] = t[2 * n] + t[2 * n + 1];
    }

    void build(vector<int> &v) { build(v, 1, 0, len - 1); }
    int query(int ql, int qr) { query(1, 0, len - 1, ql, qr); }
    void update(int rl, int rr, int val) { update(1, 0, len - 1, rl, rr, val); }
};
...
int main()
{
    ...
    cout << "After Updating, um for range 1 to 5 is: " << sum << endl;
    ...
    cout << "After Updating, um for range 2 to 3 is: " << sum << endl;
    return 0;
}